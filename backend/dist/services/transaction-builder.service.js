export class TransactionBuilderService {
    ...
    static addLogToTransaction(tx, log) {
        tx.logs.push(log);
        // Propagate variables if they appear in later lines
        if (log.patientId && !tx.patientId)
            tx.patientId = log.patientId;
        if (log.pccFacilityId && !tx.pccFacilityId)
            tx.pccFacilityId = log.pccFacilityId;
        if (log.facilityId) {
            if (!tx.facilityId || tx.facilityId === tx.pccFacilityId) {
                tx.facilityId = log.facilityId;
            }
        }
        if (log.orgUuid && !tx.orgUuid)
            tx.orgUuid = log.orgUuid;
        if (log.eventType && tx.eventType === 'unknown')
