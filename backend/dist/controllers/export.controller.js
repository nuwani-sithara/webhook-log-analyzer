...
export function exportExcel(req, res) {
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=pcc_webhook_report_${reportId}.xlsx`);
...
export function exportCsv(req, res) {
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `attachment; filename=pcc_webhook_transactions_${reportId}.csv`);
        res.send(csvContent);
    }
    catch (error) {
...
export function exportHtml(req, res) {
    const { reportId } = req.params;
    const report = reportCache.get(reportId);
    if (!report) {
        res.status(404).json({ error: 'Report not found or has expired.' });
    ...
    try {
        ...
        res.setHeader('Content-Disposition', `inline; filename=pcc_webhook_report_${reportId}.html`);
        res.send(htmlReport);
    }
    catch (error) {
