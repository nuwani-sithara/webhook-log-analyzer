...
export async function analyzeLogs(req, res) {
    const file = req.file;
    if (!file) {
    ...
    try {
        ...
        const errorsByFacilityMap = new Map();
        const warningsByDayMap = new Map();
        ...
        for (const tx of transactions) {
            ...
            if (tx.patientId)
                patientsAffected.add(tx.patientId);
            if (tx.facilityId)
                facilitiesAffected.add(tx.facilityId);
            ...
            if (tx.facilityId && tx.errors.length > 0) {
                errorsByFacilityMap.set(tx.facilityId, (errorsByFacilityMap.get(tx.facilityId) || 0) + tx.errors.length);
            }
            ...
            const d = tx.duration;
            if (d < 500)
                durationBuckets['< 500ms']++;
            else if (d < 1000)
                durationBuckets['500ms - 1s']++;
            else if (d < 2000)
                durationBuckets['1s - 2s']++;
            else if (d < 5000)
                durationBuckets['2s - 5s']++;
            else
                durationBuckets['> 5s']++;
