import { WebhookTransaction, AISummary } from '../types/index.js';
...
export class AiSummaryService {
  ...
  static async generateSummary(
    ...
    const errors: string[] = [];
    const patientsReqInv = new Set<string>();
    const facilitiesWithProblemsMap = new Map<string, { errors: number; warnings: number }>();
    const censusAnomalies: string[] = [];
    const forecastAnomalies: string[] = [];
    const duplicateWebhookDetection: string[] = [];
    ...
    for (const tx of transactions) {
      ...
      if (tx.status === 'error' || tx.status === 'warning') {
        ...
        if (tx.status === 'error') facStats.errors++;
        if (tx.status === 'warning') facStats.warnings++;
      ...
      for (const err of tx.errors) {
        if (err.toLowerCase().includes('timeout')) totalTimeouts++;
        if (err.toLowerCase().includes('token') || err.toLowerCase().includes('unauthorized')) totalTokenFailures++;
    ...
    if (totalTimeouts > 0) criticalIssues.push(`Encountered ${totalTimeouts} timeout exceptions indicating network or API threshold delays.`);
    if (totalTokenFailures > 0) criticalIssues.push(`Authentication errors: ${totalTokenFailures} API requests failed due to 401/403 credentials expire.`);
    if (totalCensusHangs > 0) criticalIssues.push(`Active service hangs: ${totalCensusHangs} PccCensusService threads halted before completing.`);
    if (totalForecastHangs > 0) criticalIssues.push(`Recalculation failures: ${totalForecastHangs} DailyForecastRecalculationService processes did not finish.`);
