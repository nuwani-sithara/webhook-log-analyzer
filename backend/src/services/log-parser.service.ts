...
export class LogParserService {
  // Regex patterns
  private static TIMESTAMP_REGEX = /^\[?(\d{4}-\d{2}-\d{2}[\sT]\d{2}:\d{2}:\d{2}[,.]\d{3}Z?)\]?/;
  private static LOG_LEVEL_REGEX = /\b(INFO|WARN|ERROR|DEBUG)\b/;
  
  // PCC Webhook IDs patterns, e.g. [wh-123-abc] or webhookId: "wh-123"
  private static WEBHOOK_ID_REGEX = /(?:\[(wh-[a-zA-Z0-9-]+)\]|webhookId[":\s]+([a-zA-Z0-9-]+)|Correlation ID:?\s*([a-zA-Z0-9-]+)|messageId[":\s]+([a-zA-Z0-9-]+))/i;
  
  // Specific entity extractors
  private static PATIENT_ID_REGEX = /(?:patient\s*(?:id|uuid)?[":\s\[]+([a-zA-Z0-9-]+)|(?:fetch-patient|patient-coverage|get-adt-records)\/([a-zA-Z0-9-]+))/i;
  private static FACILITY_ID_REGEX = /(?:facility\s*(?:id|uuid)?|facId)[":\s\[]+([a-zA-Z0-9-]+)/i;
  private static ORG_UUID_REGEX = /(?:org\s*(?:uuid|id)?[":\s\[]+([a-zA-Z0-9-]+))/i;
  private static PCC_FACILITY_ID_REGEX = /(?:pccFacilityDbURL:(\d+)|"facId":\s*(\d+))/i;
  
  // API Call patterns
  private static API_REQUEST_REGEX = /(?:PCC API Request|GET|POST|PUT|DELETE):\s*(GET|POST|PUT|DELETE)?\s*([^\s]+)/i;
  private static API_STATUS_REGEX = /Status:\s*\[?(\d{3})\]?/i;
  private static API_DURATION_REGEX = /(?:duration|took)[":\s]+(\d+)\s*ms/i;

  /**
   * Stateful method to parse all text in a PDF page.
   * Handles multi-line label-value fields and multi-line JSON blocks.
   */
  static parsePage(pageText: string, pageNum: number, startLineNum: number): LogEntry[] {
   ...
   */
  static parseLine(rawMessage: string, page: number, lineNum: number): LogEntry {
    ...
    if (lvlMatch) {
      level = lvlMatch[1].toUpperCase() as any;
