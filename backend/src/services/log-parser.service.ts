...
export class LogParserService {
  ...
  private static WEBHOOK_ID_REGEX = /(?:\[(wh-[a-zA-Z0-9-]+)\]|webhookId[":\s]+([a-zA-Z0-9-]+)|Correlation ID:?\s*([a-zA-Z0-9-]+))/i;
  
  // Specific entity extractors
  private static PATIENT_ID_REGEX = /(?:patientId[":\s]+([a-zA-Z0-9-]+)|Patient[":\s\[]+([a-zA-Z0-9-]+)\]?|\/patients\/([a-zA-Z0-9-]+))/i;
  private static FACILITY_ID_REGEX = /(?:facilityId[":\s]+([a-zA-Z0-9-]+)|Facility[":\s\[]+([a-zA-Z0-9-]+)\]?|\/facilities\/([a-zA-Z0-9-]+))/i;
  private static ORG_UUID_REGEX = /(?:orgUuid[":\s]+([a-zA-Z0-9-]+)|Org[":\s\[]+([a-zA-Z0-9-]+)\]?)/i;
  
  // API Call patterns
  private static API_REQUEST_REGEX = /PCC API Request:\s*(GET|POST|PUT|DELETE)\s*([^\s]+)/i;
